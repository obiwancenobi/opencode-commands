...
Commit all changes, push to remote, and create a pull request with user approval.
...
- `git log --oneline origin/main..HEAD 2>/dev/null || git log --oneline origin/master..HEAD 2>/dev/null || git log --oneline -10`
- `git diff origin/main...HEAD 2>/dev/null || git diff origin/master...HEAD 2>/dev/null || git diff --stat -10`
- `git diff --stat origin/main...HEAD 2>/dev/null || git diff --stat origin/master...HEAD 2>/dev/null || echo ""`

### 8. Detect Base Branch
- Use `execute_command` to run `git remote show origin 2>/dev/null | grep 'HEAD branch' | awk '{print $NF}'`
...
- Use `execute_command` to run `which gh 2>/dev/null`
- If not found:
  - Use `execute_command` to run `brew --version 2>/dev/null` (check if Homebrew available)
  - Use `ask_user`:
  ...
  - If user chooses "Install gh": Use `execute_command` to run `brew install gh`, then check auth
  - If install fails or user chooses "Open in Browser": go to step 13
- If found, check auth:
  - Use `execute_command` to run `gh auth status 2>&1`
  - If not authenticated:
    ...
    > 2. Open in Browser — Skip gh, open PR compare URL in browser to create manually
    > 3. Cancel — Abort without creating PR
  - If user chooses "Login to gh": Use `execute_command` to run `gh auth login`, then proceed
...
- Use `execute_command` to run `gh pr create --title "TITLE" --body "BODY" --base BASE_BRANCH`
...
- Use `execute_command` to run `open "COMPARE_URL"` (macOS) or `xdg-open "COMPARE_URL"` (Linux)
