...
Generate social media content from user-provided context and distribute across selected platforms using parallel subtasks. Follow these steps:
...
1. Use `execute_command` to run `cat README.md`
2. If no README found, ask user to provide context manually
3. Use the README content as the topic context
...
Wait for user response. Map number to timezone if applicable, or use the custom timezone directly.
...
1. Derive a kebab-case directory name from `{TOPIC}`:
...
For each selected platform, launch a parallel subtask using `new_task()`. All subtask calls should be initiated simultaneously for parallel execution.

For each platform, call `new_task()` with a prompt containing the platform-specific content generation instructions below.

### LinkedIn Subtask Prompt
...
- **Subtask failed**: Report which platform subtask failed and suggest re-running for that platform individually.
